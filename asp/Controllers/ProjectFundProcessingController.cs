...
{
    [ApiController]
    [Route("api/project-fund-processing")]
    public class ProjectFundProcessingController : Controller
    {
        private readonly ProjectFundProcessingService _resp;


       
        public ProjectFundProcessingController(ProjectFundProcessingService resp )
        {
            _resp = resp;
        }
        [HttpPost]
        public async Task<IActionResult> Create([FromForm] ProjectFundsProcessing request)
        {
            if (request == null)
            {
        ...
        [HttpGet()]
        public async Task<IActionResult> GetAllProjectFunds(int page = 1, int size = 10, string filterType = ApprovedConst.PROCESSING, string? userId = "", string? searchValue = null)
        {
            var skipAmount = (page - 1) * size;
            List<ProjectFundsProcessing> datas;
            long totalProjectFunds;
            // Kiểm tra xem userId có được truyền hay không
            if (string.IsNullOrEmpty(userId))
            {
                datas = await _resp.GetAllAsync(skipAmount, size, filterType, "", searchValue);
            }
            else
            {
                datas = await _resp.GetAllAsync(skipAmount, size, filterType, userId, searchValue);
            }
        ...
        public async Task<IActionResult> UpdateApprovalStatus([FromForm] UpdateApprovalStatusDTO dto)
        {
            try
            {
                var isUpdated = await _resp.UpdateApprovalStatusAsync(dto);
                if (isUpdated)
                {
                    return Ok(new ApiResponseDTO<object> { data = new { success = "Sucess" }, message = "Duyệt dự án thành công." });
                }
                else
                {
                    return BadRequest(new ApiResponseDTO<object> { data = new { error = "Error" }, message = "Duyệt dự án thất bại." });
                }
            }
            catch (Exception ex)
            {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> updateProjectFund(string id, [FromForm] ProjectFundsProcessing updatedProjectFund)
        {
